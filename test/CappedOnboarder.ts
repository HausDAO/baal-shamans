...
const summonOnboarder = async function (
  onboarderArgs: any,
  multisend: MultiSend,
  onboarderSingleton: CappedOnboarderShaman,
  onboarderSummoner: CappedOnboarderShamanSummoner,
  baal: Baal
) {
  ...
  let result = await summonOnboarder1.wait() as any;
  if (
    // result &&
    // result.events &&
    // result.events[1] &&
...
describe("Onboarder", function () {
  ...
  let onboarderSingleton: CappedOnboarderShaman;
  let OnboarderSummonerFactory: ContractFactory;
  let onboarderSummoner: CappedOnboarderShamanSummoner;
  let onboarder: CappedOnboarderShaman;
  ...
  this.beforeAll(async function () {
    ...
    OnboarderFactory = await ethers.getContractFactory("CappedOnboarderShaman");
    onboarderSingleton =
      (await OnboarderFactory.deploy()) as CappedOnboarderShaman;
  ...
  beforeEach(async function () {
    ...
    await token.transfer(
      applicant.address,
      ethers.utils.parseUnits("2000.0", "ether")
    );
    await token.transfer(s2.address, ethers.utils.parseUnits("2000.0", "ether"));
  ...
  describe("capped onboarder", function () {

    it.only("mint shares on sending token", async function () {
    ...
    it.only("mint loot on sending over cap", async function () {
      const amount = ethers.utils.parseUnits("120.0", "ether")
      const onboarderArgs = {
        token: token.address,
        expiery: Math.floor(Date.now() / 1000) + 86400 * 365,
        cap: ethers.utils.parseUnits("1000", "ether"),
      ...
      const s2SharesBefore = await sharesToken.balanceOf(s2.address);
      const baalTotalSupplyBefore = await baal.totalSupply();
      console.log('s2BalanceBefore', s2BalanceBefore);
      console.log('s2LootBefore', s2LootBefore);
      console.log('baalTotalSupplyBefore', baalTotalSupplyBefore);
      ...
      const baalTotalShares = await baal.totalShares();
      console.log('s2BalanceAfter', s2BalanceAfter);
      console.log('s2LootAfter', s2LootAfter);
      console.log('baalTotalSupplyAfter', baalTotalSupplyAfter);
      ...
      expect(s2LootAfter).to.equal(
        amount.mul(onboarderArgs.multiplier).sub(onboarderArgs.cap)
      );
    });
    it.only("tribute sends 1% shares", async function () {
      ...
      const s2LootAfter = await lootToken.balanceOf(s2.address);
      const s2SharesAfter = await sharesToken.balanceOf(s2.address);
      const ecoLootAfter = await lootToken.balanceOf("0x4A9a27d614a74Ee5524909cA27bdBcBB7eD3b315");
      const baalTotalSupplyAfter = await baal.totalSupply();
      const baalTotalShares = await baal.totalShares();
      console.log('s2BalanceAfter', s2BalanceAfter);
      console.log('s2LootAfter', s2LootAfter);
